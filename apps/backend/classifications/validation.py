...
def validate_score_distribution(
    ...
    *,
    ...
    for field_name, value in (
        (f"{profile_label} Micro", micro),
        (f"{profile_label} Mystiko", mystiko),
        (f"{profile_label} Macro", macro),
    ):
        # Reject booleans (bool is a subclass of int).
        if isinstance(value, bool):
            errors.setdefault(field_name, []).append(
                _("%(label)s must be an integer, not a boolean.")
                % {"label": field_name}
            )
            continue
        ...
        if not isinstance(value, int):
            errors.setdefault(field_name, []).append(
                _("%(label)s must be an integer.") % {"label": field_name}
            )
            continue
